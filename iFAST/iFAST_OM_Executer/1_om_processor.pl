...
#All true/Any true
    ...
    if (-e "$om_file"){
        ...
        while(<OM>) {
            if ($_ !~ /VZ_/ ) { next; }
            my($hour,$gNB_ID,$NE_ID,$S5NR_DLMACLayerDataVolume_MB,$S5NR_ULMACLayerDataVolume_MB,$S5NR_ULMACcellAggregateTput_Mbps_Num,$S5NR_DLTTIUtilization_Pct_Num) = (split(',', $_))[2,3,4,22,23,37,39];
            # if ($enb_id_noDup eq "$given_NE"){
                ...
                # my ($first_num_D) = $cell_id =~ /(\d+)/;#for cbrs
                 if(($hour == $cst_prev_Hour) && ( grep( /^$NE_ID$/, @siteList ) )){
                    $status_hash_noDup{$gNB_ID}{$NE_ID}{S5NR_DLMACLayerDataVolume_MB} += $S5NR_DLMACLayerDataVolume_MB;
                    $status_hash_noDup{$gNB_ID}{$NE_ID}{S5NR_ULMACLayerDataVolume_MB} += $S5NR_ULMACLayerDataVolume_MB;
                    $status_hash_noDup{$gNB_ID}{$NE_ID}{S5NR_ULMACcellAggregateTput_Mbps_Num} += $S5NR_ULMACcellAggregateTput_Mbps_Num;
                    $status_hash_noDup{$gNB_ID}{$NE_ID}{S5NR_DLTTIUtilization_Pct_Num} += $S5NR_DLTTIUtilization_Pct_Num;
                 }
            # }
        }
        close(NRPRACH);
    ...
    print (CANDIDATES "GNB_ID,DU_ID,STATUS\n");
    print (LOG Dumper( \%status_hash_noDup));
    print (LOG "GNB_ID,DU_ID,CONDITION,PROCESSED_VALUE,OPERATOR,COMPARE_VALUE\n");

    #formulas
    foreach my $gNB_ID (keys %status_hash_noDup) {
        #print "$NE_ID\n";
        my $status="FAIL";
        my @boolArr=();
        foreach my $NE_ID ( keys %{$status_hash_noDup{$gNB_ID}}) {
            #print "$cell_id\n";
            ...
            while(<FILE_D>) {
                ...
                if((index($condLine,"+")!= -1)&& (scalar(@cond_arr)==2)){
                    ...
                    $cond=($status_hash_noDup{$gNB_ID}{$NE_ID}{$cond_arr[0]} + $status_hash_noDup{$gNB_ID}{$NE_ID}{$cond_arr[1]});
                }elsif((index($condLine,"-")!= -1)&& (scalar(@cond_arr)==2)){
                    # $cond=$cond_arr[0]-$cond_arr[1];
                    $cond=($status_hash_noDup{$gNB_ID}{$NE_ID}{$cond_arr[0]} - $status_hash_noDup{$gNB_ID}{$NE_ID}{$cond_arr[1]});
                }
                if((scalar(@cond_arr)==1)){
                    $cond=$status_hash_noDup{$gNB_ID}{$NE_ID}{$cond_arr[0]};
                }

                #only 1 operand?
                #print "NE_ID=$NE_ID,cell_id=$cell_id,condLine=$condLine,processedValue=$cond,operator=$operator,CompareValue=$value\n";
                print (LOG "$gNB_ID,$NE_ID,$condLine,$cond,$operator,$value\n");
                if($operator eq "="){
                    if($cond == $value){
                        push(@boolArr,1);
                        #print "pass\n";
            ...
            if (@boolArr == grep { $_ eq "1" } @boolArr) {
                ...
                print (CANDIDATES "$gNB_ID,$NE_ID,PASS\n");
