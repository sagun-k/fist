...
class InsertIssueSeqService:
    ...
    def update_issue_sequence(self, issue_id, sw_val, act_type, status_val, scname, seq_id,args, c_type, proceed, c_cond, grep, om_counters, ip, action_id, prev_act_type, uid, pwd, session):
        ...
        if not self.is_action_type_changed(act_type, action_id, exec_orders):
            ...
            return self.update_when_action_type_changed(sw_val, act_type, status_val, scname, seq_id, args, c_type, proceed, c_cond, grep, om_counters, ip, action_id, prev_act_type, issue_id, uid, pwd,session)        
    ...
    def update_when_action_type_changed(self, sw_val, act_type, status_val, scname, seq_id,args, c_type, proceed, c_cond, grep, om_counters, ip, action_id, prev_act_type, issue_id, uid, pwd, session):
        exec_orders = self.get_exec_orders(issue_id)
        previous_existed_exec_id = self.map_action_type_id(prev_act_type, action_id)
        current_exec_position = self.find_index_of_current_exec(exec_orders, previous_existed_exec_id)
        self.delete_existed_exec(prev_act_type, action_id)
    ...
    def delete_existed_exec(self, prev_act_type, action_id):
        DB_TABLE = self.ACTION_DB_MAPPPING.get(prev_act_type, None)
        delete_query = f"DELETE FROM {DB_TABLE} WHERE {action_id}=%s "
        self.db.execute_query(delete_query, action_id, commit=True)
    ...
    def is_action_type_changed(self, action_type, action_id, exec_orders):
        exec_id = self.map_action_type_id(action_type, action_id)
        exec_index = self.find_index_of_current_exec(exec_orders, exec_id)
        return exec_index == -1
