...
#All true/Any true
    ...
    if (-e "$nr_om_file"){
        ...
        open (NRPRACH, "<$nr_om_file");
        #open (NRPRACH, "<Logs/NR_PRACH_BEAM_pidXXX_06-02-2024_161230.csv");
        while(<NRPRACH>) {
            if ($_ !~ /CellIdentity/ ) { next; }
            my($NE_NAME, $cellLocation, $RachPreambleAPerBeam, $RachPreambleACFRAPerBeam, $NumMSG3PerBeam, $NumMSG3CFRAPerBeam) = (split(',', $_))[0,1,2,3,6,7];
            # if ($enb_id_noDup eq "$given_NE"){
                ...
                # if($first_num_D < 100){
                    $status_hash_noDup{$NE_ID}{$cell_id}{RachPreambleAPerBeam} += $RachPreambleAPerBeam;
                    $status_hash_noDup{$NE_ID}{$cell_id}{RachPreambleACFRAPerBeam} += $RachPreambleACFRAPerBeam;
                    $status_hash_noDup{$NE_ID}{$cell_id}{NumMSG3PerBeam} += $NumMSG3PerBeam;
                    $status_hash_noDup{$NE_ID}{$cell_id}{NumMSG3CFRAPerBeam} += $NumMSG3CFRAPerBeam;
                # }
            # }
        }
        close(NRPRACH);
    ...
    # if (-e "$omfiles[0]"){
        # open (CUCPCSL, "<$omfiles[1]");
        # while(<CUCPCSL>) {
            # if ($_ !~ /CellIdentity/ ) { next; }
            # my($NE_NAME, $cellLocation, $CauseInternalRandomAccessProblem) = (split(',', $_))[0,1,29];
            # # if ($enb_id_noDup eq "$given_NE"){
                # my $cell_id = $cellLocation;
                # my($NE_ID, $loc_id) = (split('_', $NE_NAME))[0,1];
                # # my ($first_num_D) = $cell_id =~ /(\d+)/;#for cbrs
                # # if($first_num_D < 100){
                    # $status_hash_noDup{$NE_ID}{$cell_id}{CauseInternalRandomAccessProblem} = $CauseInternalRandomAccessProblem;
                # # }
            # # }
        # }
        # close(CUCPCSL);
