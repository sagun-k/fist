...
sub EndcDrop_5g{
	if (-e "$TempPath/$cucpCslHexa_5gFile") {
		   ...
		   while(<CUCP>) {
			...
			my($du_id, $cell_id) = (split('/', $cellLocation))[1,2];
			#print "gNBId =$du_id\n";
...
sub Sgnb_5g{
	...
	if (-e "$TempPath/$SgNBAddition_5gFile") {
		...
		while(<SGNB>) {
			...
			my($du_id, $cell_id) = (split('/', $cellLocation))[1,2];#has no duplicate cell ID
			# my $DU_NUM = $given_NE;
			# $DU_NUM =~ s/DU_/gNB_DU_ID/g;
			#if (($du_id eq "$given_gNBId") && ($DU_NUM eq $cell_id)){
			if ((index($NE_name, $given_AUID) != -1) || ($du_id eq $given_NE)) {			
...
sub interIntra {
	if (-e "$TempPath/$intraSnPscell_5gFile") {
		...
		while(<INTRA>) {
			...
			my($du_id, $cell_id) = (split('/', $cellLocation))[1,2];#has duplicate cell ID's
			# my $DU_NUM = $given_NE;
			# $DU_NUM =~ s/DU_/gNB_DU_ID/g;
			$cell_id =~ s/Src//g;
			#if ($du_id eq "$given_gNBId"){
